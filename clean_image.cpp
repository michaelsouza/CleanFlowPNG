        ...
        for (int i = 0; i < m; ++i)
        {
            for (int j = 0; j < n; ++j)
            {
                if (matrix[i][j] == WHITE || matrix[i][j] == ORANGE)
                {
                    ...
                    int max_freq = -1;
                    int max_color = matrix[i][j];
                    for (size_t color = 0; color < freq.size(); ++color)
                    {
                        // if there is a tie, btw green and red, restore the original color
                        if ((freq[color] == max_freq) && (max_color == GREEN) && (color == RED))
                        {                            
                            max_color = matrix[i][j];
                        }
                        else if (freq[color] > max_freq)
                        {
                            max_freq = freq[color];
                            max_color = color;
                        }
                    ...
                    if (max_color != matrix[i][j])
                    {
                        matrix[i][j] = max_color;
                        updated = true;
                    }
                }
            }
